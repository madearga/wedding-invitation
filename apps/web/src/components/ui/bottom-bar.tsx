 ...
 * with auto-detection of active sections using Intersection Observer
 *
...
const BottomBar = () => {
  ...
  const handleMenuClick = useCallback((e: React.MouseEvent<HTMLAnchorElement>, href: string, id: string) => {
    e.preventDefault();
          ...
          <nav className="flex items-center gap-1">
            {menuItems.map((item) => (
              <motion.a
                key={item.id}
                href={item.href}
                className="flex flex-col items-center justify-center px-3 py-2 rounded-xl transition-all duration-200 hover:bg-gray-50/50"
